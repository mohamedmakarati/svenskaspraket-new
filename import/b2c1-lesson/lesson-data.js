window.LESSON_DATA = {
  "title": "Rivstart B2/C1 - Kapitel 1-3: ord och verb",
  "creatorCredit": "Quizlet-set skapade av AnnaRansheim",
  "disclaimer": "En oberoende studieanpassning. Inte officiellt material från Quizlet, Rivstart eller förlaget.",
  "sets": [
    {
      "id": "chapter1",
      "title": "Så funkar ett språkgeni",
      "sourceUrl": "https://quizlet.com/_8nlhmv",
      "entries": [
        [
          1,
          "avslöjar (ngt FÖR ngn)",
          "reveals",
          "يكشف (شيئاً لشخص)"
        ],
        [
          2,
          "utmaning-en (MED ngt FÖR ngn)",
          "challenge",
          "تحدٍّ"
        ],
        [
          3,
          "bevisar",
          "proves",
          "يثبت"
        ],
        [
          4,
          "uppdrag-et",
          "mission",
          "مهمة؛ تكليف"
        ],
        [
          5,
          "utrustad",
          "equipped",
          "مُجهَّز"
        ],
        [
          6,
          "gör sitt yttersta do his/her",
          "utmost",
          ""
        ],
        [
          7,
          "slukar",
          "devours",
          "يلتهم"
        ],
        [
          8,
          "utförlig",
          "detailed",
          "مفصّل"
        ],
        [
          9,
          "drygt",
          "just over",
          "أكثر بقليل من"
        ],
        [
          10,
          "uppskattar",
          "estimates",
          "يقدّر"
        ],
        [
          11,
          "enastående",
          "unique",
          "استثنائي؛ متميز"
        ],
        [
          12,
          "ytterligare",
          "further",
          "إضافي؛ مزيد من"
        ],
        [
          13,
          "begåvning-en",
          "talent",
          "موهبة"
        ],
        [
          14,
          "ansträngning-en",
          "effort",
          "مجهود"
        ],
        [
          15,
          "nämligen",
          "namely",
          "أي؛ تحديداً"
        ],
        [
          16,
          "förknippar",
          "associate",
          "يربط؛ يقترن في ذهنه"
        ],
        [
          17,
          "upplevelse-n",
          "experience",
          "تجربة"
        ],
        [
          18,
          "snäv",
          "narrow",
          "ضيّق؛ محدود"
        ],
        [
          19,
          "myndighet-en",
          "administrative authority",
          "جهة حكومية؛ سلطة"
        ],
        [
          20,
          "egenskap-en",
          "quality",
          "صفة؛ خاصية"
        ],
        [
          21,
          "uteslutande",
          "excludingly",
          "حصرياً؛ فقط"
        ],
        [
          22,
          "mönster (mönstret)",
          "pattern",
          "نمط"
        ],
        [
          23,
          "tillvaro-n",
          "existence",
          "وجود؛ حياة"
        ],
        [
          24,
          "verksam",
          "busy",
          "نشِط؛ عامل"
        ],
        [
          25,
          "föreläsare-n",
          "lecturer",
          "محاضر"
        ],
        [
          26,
          "utarbetar",
          "elaborates",
          "يُعِدّ؛ يطوّر بالتفصيل"
        ],
        [
          27,
          "skiner",
          "upp smile, look happy",
          ""
        ],
        [
          28,
          "(syskon-)skara-n host (of siblings)",
          "",
          ""
        ],
        [
          29,
          "anfall-et",
          "seizure",
          "نوبة؛ هجوم"
        ],
        [
          30,
          "tillflykt-en",
          "refuge",
          "ملجأ"
        ],
        [
          31,
          "färdighet-en",
          "skill",
          "مهارة"
        ],
        [
          32,
          "anstränger",
          "sig makes an effort",
          ""
        ],
        [
          33,
          "förmedlar",
          "conveys",
          "ينقل؛ يوصّل"
        ],
        [
          34,
          "känslomässig",
          "emotive",
          "عاطفي"
        ],
        [
          35,
          "gemenskap-en",
          "community",
          "مجتمع؛ شعور بالانتماء"
        ],
        [
          36,
          "utvecklar",
          "develops",
          "يطوّر"
        ],
        [
          37,
          "kommer på",
          "thinks of",
          "يخطر بباله؛ يدرك"
        ],
        [
          38,
          "meningsfull",
          "meaningful",
          "ذو معنى"
        ],
        [
          39,
          "anmärkningsvärd",
          "remarkable",
          "لافت للنظر"
        ],
        [
          40,
          "irrar",
          "runt",
          ""
        ],
        [
          41,
          "livnär",
          "sig makes a living",
          ""
        ],
        [
          42,
          "inriktar sig (på)",
          "concentrate upon",
          "يركّز على"
        ],
        [
          43,
          "omgivning-en",
          "(here) people around you, surroundings",
          "المحيط؛ الأشخاص من حولك"
        ],
        [
          44,
          "utlöser",
          "produces, sparks",
          "يُطلِق؛ يسبّب"
        ],
        [
          45,
          "uppmuntrar",
          "encourages",
          "يشجّع"
        ],
        [
          46,
          "ställer",
          "upp helps",
          ""
        ],
        [
          47,
          "kartlägger",
          "maps out",
          "يرسم خريطة؛ يحصر ويدرس"
        ],
        [
          48,
          "håller isär",
          "disassociates",
          "يفصل بين؛ يميّز"
        ],
        [
          49,
          "glosa-n",
          "word",
          "مفردة؛ كلمة للحفظ"
        ],
        [
          50,
          "kännetecken (kännetecknet)",
          "characteristic",
          "سمة مميّزة"
        ],
        [
          51,
          "i hög",
          "grad largely",
          ""
        ],
        [
          52,
          "besitter",
          "has, possesses",
          "يمتلك"
        ],
        [
          53,
          "behärskar",
          "masters",
          "يتقن"
        ],
        [
          54,
          "avrundar",
          "round off",
          "يختتم؛ يقرّب الرقم"
        ],
        [
          55,
          "kommer överens",
          "agrees",
          "يتفق؛ ينسجم"
        ],
        [
          56,
          "underbarn-et",
          "prodigy",
          "طفل معجزة؛ نابغة"
        ],
        [
          57,
          "ödmjuk",
          "humble",
          "متواضع"
        ],
        [
          58,
          "främst",
          "mainly",
          "بشكل رئيسي؛ أولاً"
        ],
        [
          59,
          "får utlopp för",
          "",
          "يجد متنفساً لـ"
        ],
        [
          60,
          "förtjusning-en",
          "delight, excitement",
          "بهجة؛ سرور"
        ],
        [
          61,
          "i",
          "regel as a rule",
          ""
        ]
      ]
    },
    {
      "id": "verbs1to3",
      "title": "Verb, kapitel 1-3",
      "sourceUrl": "https://quizlet.com/_8n36aw",
      "entries": [
        [
          1,
          "anar",
          "have a feeling, suspect, anticipate",
          "يشعر؛ يشتبه؛ يتوقع"
        ],
        [
          2,
          "beror (PÅ ngt/ngn)",
          "depend",
          "يعتمد على"
        ],
        [
          3,
          "bryr sig (OM ngn/ngt)",
          "care for",
          "يهتم بـ"
        ],
        [
          4,
          "knäcker",
          "koden crack, break, snap",
          ""
        ],
        [
          5,
          "kräver",
          "demand, require",
          "يطلب؛ يتطلب"
        ],
        [
          6,
          "lossnar",
          "come loose, come off, come undone",
          "ينفك؛ ينفصل"
        ],
        [
          7,
          "når",
          "reach, attain",
          "يصل؛ يحقق"
        ],
        [
          8,
          "sker",
          "happen, occur",
          "يحدث"
        ],
        [
          9,
          "släpper",
          "release, let go",
          "يُطلِق؛ يترك"
        ],
        [
          10,
          "smäller",
          "slam, bang",
          "يصفق بقوة؛ يُحدث دوياً"
        ],
        [
          11,
          "stöter",
          "strike, poke, push",
          "يصطدم؛ يدفع؛ ينكز"
        ],
        [
          12,
          "upplever",
          "experience, feel, think",
          "يختبر؛ يشعر"
        ],
        [
          13,
          "uppskattar",
          "appreciate, estimate",
          "يقدّر؛ يُثمّن"
        ],
        [
          14,
          "upptäcker",
          "discover, detect",
          "يكتشف"
        ],
        [
          15,
          "ägnar (sig/ngt ÅT ngn/ngt) devote, dedicate (time, life, effort)",
          "",
          ""
        ],
        [
          16,
          "anförtro (sig ÅT ngn) confide, entrust (smth to smb)",
          "",
          ""
        ],
        [
          17,
          "avskyr",
          "hate, detest",
          "يكره بشدة"
        ],
        [
          18,
          "beter",
          "sig behave",
          ""
        ],
        [
          19,
          "flyr",
          "escape",
          "يهرب"
        ],
        [
          20,
          "glor",
          "stare, glare, gape",
          "يحدّق"
        ],
        [
          21,
          "snor",
          "twist; pinch (informal)",
          "يلوي؛ يسرق (عامية)"
        ],
        [
          22,
          "spyr",
          "throw up, puke",
          "يتقيأ"
        ],
        [
          23,
          "strör scatter, sprinkle (sugar, flower, salt)",
          "",
          ""
        ],
        [
          24,
          "avviker,",
          "avvek, avvikit differ, deviate*",
          ""
        ],
        [
          25,
          "ber,",
          "bad, bett beg, request; pray*",
          ""
        ],
        [
          26,
          "består, bestod, bestått (AV ngt)",
          "consist of*",
          "يتكوّن من"
        ],
        [
          27,
          "driver,",
          "drev, drivit force, impel; carry on*",
          ""
        ],
        [
          28,
          "knyter, knöt,",
          "knutit tie, fasten, knot*",
          ""
        ],
        [
          29,
          "pågår, pågick, pågått",
          "continue, last, proceed",
          "يستمر؛ يجري"
        ],
        [
          30,
          "rinner,",
          "rann, runnit flow, streem, pour*",
          ""
        ],
        [
          31,
          "sliter,",
          "slet, slitit tear, pull; work hard*",
          ""
        ],
        [
          32,
          "slår,",
          "slog, slagit hit*",
          ""
        ],
        [
          33,
          "sprider,",
          "spred, spridit spread*",
          ""
        ],
        [
          34,
          "umgås, umgicks, umgåtts",
          "socialize, spend time together*",
          "يتواصل اجتماعياً؛ يقضي وقتاً مع"
        ],
        [
          35,
          "uppger, uppgav, uppgett (uppgivit) declare, give (information)*",
          "",
          ""
        ],
        [
          36,
          "behåller, behöll, behållit",
          "keep*",
          "يحتفظ بـ"
        ],
        [
          37,
          "förhåller sig, förhöll sig, förhållit sig remain (passive, negative),",
          "have an attitude*",
          ""
        ],
        [
          38,
          "ger",
          "sig ut, gav, gett (givit) go out",
          ""
        ],
        [
          39,
          "undviker,",
          "undvek, undvikit avoid*",
          ""
        ],
        [
          40,
          "utgår, utgick, utgått (FRÅN ngt)",
          "emanate; assume",
          "ينطلق من؛ يفترض"
        ],
        [
          41,
          "återupptar, återupptog, återupptagit",
          "take upp (smth) again, resume",
          "يستأنف"
        ],
        [
          42,
          "beställer",
          "order",
          "يطلب"
        ],
        [
          43,
          "brer",
          "spread; make a sandwich",
          "يدهن؛ يحضّر شطيرة"
        ],
        [
          44,
          "flår",
          "skin, flay",
          "يسلخ"
        ],
        [
          45,
          "fyller",
          "fill",
          "يملأ"
        ],
        [
          46,
          "för (förde, fört)",
          "transport, convey",
          ""
        ],
        [
          47,
          "stjäl,",
          "stal, stulit steal*",
          ""
        ],
        [
          48,
          "spricker,",
          "sprack, spruckit crack, split, burst",
          ""
        ],
        [
          49,
          "styr (styrde, styrt)",
          "steer, direct",
          ""
        ],
        [
          50,
          "stör (störde, stört)",
          "disturb, interrupt",
          ""
        ],
        [
          51,
          "bär,",
          "bar, burit carry*",
          ""
        ],
        [
          52,
          "dör, dog, dött",
          "die",
          "يموت"
        ],
        [
          53,
          "förblir, förblev, förblivit remain, stay (the same way)",
          "",
          ""
        ],
        [
          54,
          "hinner, hann, hunnit have time to do (smth)*",
          "",
          ""
        ],
        [
          55,
          "lider,",
          "led, lidit suffer, be in pain*",
          ""
        ],
        [
          56,
          "lägger,",
          "la, lagt put, place*",
          ""
        ],
        [
          57,
          "ser, såg,",
          "sett see*",
          ""
        ],
        [
          58,
          "sjunker, sjönk,",
          "sjunkit go down, sink*",
          ""
        ],
        [
          59,
          "skär,",
          "skar, skurit cut",
          ""
        ],
        [
          60,
          "slipper,",
          "slapp, sluppit not have to",
          ""
        ],
        [
          61,
          "säljer, sålde, sålt",
          "sell",
          "يبيع"
        ],
        [
          62,
          "sätter,",
          "satte, satt put, place, seat",
          ""
        ]
      ]
    },
    {
      "id": "ordOrdOrd",
      "title": "Ord, ord, ord",
      "sourceUrl": "https://quizlet.com/_8n3678",
      "entries": [
        [
          1,
          "betecknar",
          "stands for",
          "يعني؛ يرمز إلى"
        ],
        [
          2,
          "formulerar",
          "expresses",
          "يعبّر؛ يصوغ"
        ],
        [
          3,
          "rådande",
          "prevailing",
          "سائد؛ رائج"
        ],
        [
          4,
          "godtycklig",
          "arbitrary",
          "اعتباطي"
        ],
        [
          5,
          "överenskommelse-n",
          "agreement",
          "اتفاق"
        ],
        [
          6,
          "förknippar",
          "associate",
          "يربط؛ يقترن في ذهنه"
        ],
        [
          7,
          "föreställning-en",
          "idea, conception",
          "فكرة؛ تصور"
        ],
        [
          8,
          "belägg-et",
          "support (for)",
          "دليل؛ سند"
        ],
        [
          9,
          "härmar",
          "imitates",
          "يقلّد"
        ],
        [
          10,
          "djurläte-t",
          "animal call",
          "صوت حيوان"
        ],
        [
          11,
          "beteckning-en",
          "denomination",
          "تسمية؛ دلالة"
        ],
        [
          12,
          "upplever",
          "experiences",
          "يختبر؛ يشعر"
        ],
        [
          13,
          "representerar",
          "represent",
          "يمثل"
        ],
        [
          14,
          "uppmärksammar",
          "notices",
          "يلاحظ"
        ],
        [
          15,
          "rörelse-n",
          "movement",
          "حركة"
        ],
        [
          16,
          "nedsättande",
          "disparaging",
          "ازدرائي؛ تحقيري"
        ],
        [
          17,
          "innebär",
          "means",
          "يعني؛ يتضمن"
        ],
        [
          18,
          "påhittad",
          "made up, fictitious",
          "مختلَق؛ وهمي"
        ],
        [
          19,
          "uppger",
          "states",
          "يذكر؛ يفيد"
        ],
        [
          20,
          "klang-en",
          "tone",
          "نبرة؛ صوت"
        ],
        [
          21,
          "jämförelse-n",
          "comparison",
          "مقارنة"
        ],
        [
          22,
          "omvänt",
          "vice versa",
          "بالعكس"
        ],
        [
          23,
          "skiljer sig (åt)",
          "is distinguished from",
          "يختلف عن"
        ],
        [
          24,
          "uppsättning-en",
          "set",
          "مجموعة؛ تشكيلة"
        ],
        [
          25,
          "nämner",
          "mentions",
          "يذكر"
        ],
        [
          26,
          "tillåter",
          "permits",
          "يسمح"
        ],
        [
          27,
          "uppvisar",
          "show",
          "يعرض؛ يُظهر"
        ],
        [
          28,
          "oändlig",
          "infinite",
          "لا نهائي"
        ]
      ]
    },
    {
      "id": "quizlet8mq3xa",
      "title": "Priset på vatten",
      "sourceUrl": "https://quizlet.com/_8mq3xa",
      "entries": [
        [
          1,
          "ANSTRÄNGNING-EN",
          "effort",
          "مجهود"
        ],
        [
          2,
          "GE UPP",
          "give up",
          ""
        ],
        [
          3,
          "tar (här)",
          "has effect",
          ""
        ],
        [
          4,
          "avsikt-en",
          "intention",
          ""
        ],
        [
          5,
          "genväg-en",
          "shortcut",
          ""
        ],
        [
          6,
          "FÖREDRAR",
          "prefers",
          ""
        ],
        [
          7,
          "LÄGGER TILL",
          "adds",
          ""
        ],
        [
          8,
          "LÄGGER NER",
          "shuts down",
          ""
        ],
        [
          9,
          "de sköna konsterna",
          "the fine arts",
          ""
        ],
        [
          10,
          "OMEDELBAR",
          "immediate",
          ""
        ],
        [
          11,
          "skadar",
          "hurts",
          ""
        ]
      ]
    },
    {
      "id": "lek",
      "title": "Lek!",
      "sourceUrl": "https://quizlet.com/_8ml0cb",
      "entries": [
        [
          1,
          "fnittrar",
          "giggles",
          ""
        ],
        [
          2,
          "hejdlös",
          "irrepressible",
          ""
        ],
        [
          3,
          "fattas",
          "is lacking",
          ""
        ],
        [4, "strunt(a) i det", "disregard", ""],
        [5, "pågår", "is going on", ""],
        [6, "yrkesliv-et", "working life", ""],
        [7, "går till", "comes about", ""],
        [8, "nyss", "only a moment ago", ""],
        [9, "visst", "certainly", ""],
        [10, "undersöker", "examines", ""],
        [11, "byter ut", "changes", ""],
        [12, "noggrann, noggrant", "careful", ""],
        [13, "får syn på", "catches sight of", ""],
        [
          14,
          "samspel-et",
          "interaction",
          ""
        ],
        [
          15,
          "flink",
          "deft",
          ""
        ],
        [
          16,
          "begriplig",
          "understandable",
          ""
        ],
        [17, "stiger in i", "enters", ""],
        [
          18,
          "stöd-et",
          "support",
          ""
        ],
        [
          19,
          "knyter",
          "establishes",
          ""
        ],
        [20, "tar till sig", "takes in", ""],
        [
          21,
          "anammar",
          "adopt",
          ""
        ],
        [
          22,
          "knäcker",
          "(kod-en) breaks",
          ""
        ],
        [
          23,
          "nöjer sig (med ngt)",
          "is satisfied with",
          ""
        ],
        [
          24,
          "lyckas",
          "succeed",
          ""
        ],
        [
          25,
          "stelnar",
          "gets stiff",
          ""
        ],
        [
          26,
          "tålamod-et",
          "patience",
          ""
        ],
        [
          27,
          "ägnar sig åt",
          "is engaged in",
          ""
        ],
        [
          28,
          "spädbarn-et",
          "baby",
          ""
        ],
        [
          29,
          "omgivning-en",
          "environment",
          "المحيط؛ الأشخاص من حولك"
        ],
        [30, "består av", "is composed of", ""],
        [
          31,
          "gynnsam",
          "favourable",
          ""
        ],
        [
          32,
          "samvaro-n",
          "being together",
          ""
        ],
        [
          33,
          "lekfull",
          "playful",
          ""
        ],
        [34, "helt enkelt", "simply", ""],
        [
          35,
          "oavsett",
          "no matter, whether",
          ""
        ],
        [
          36,
          "ytterligare",
          "further",
          "إضافي؛ مزيد من"
        ],
        [
          37,
          "tidskrävande",
          "time-consuming",
          ""
        ],
        [
          38,
          "njutning-en",
          "pleasure",
          ""
        ],
        [
          39,
          "hyfsad",
          "reasonable",
          ""
        ],
        [
          40,
          "föreställning-en",
          "idea, conception",
          "فكرة؛ تصور"
        ]
      ]
    }
  ]
};
