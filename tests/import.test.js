...
describe('content import', () => {
  it('loads expected record counts from source files', async () => {
    ...
    expect(data.verbs.length).toBe(131 + 9 + 199 + 135);
    expect(data.vocabulary.length).toBe(805);
    expect(data.lessons.length).toBe(5);
  ...
  it('validates dataset with no hard failures on slugs', async () => {
    ...
    expect(report.summary.validVerbs).toBe(474);
    expect(report.summary.validVocabulary).toBe(805);
    expect(report.summary.validLessons).toBe(5);
